...
$totalPrice = sumTotalPrice($order_array, $userLogin["u_id"]);
...
$sql = "SELECT COUNT(*) AS total_rows FROM `order` WHERE u_id = '{$userLogin['u_id']}' AND o_quantity > 0 AND o_status = 1";
...
foreach ($order_array as $item) {
    if ($item['u_id'] == $userLogin['u_id'] && $item["o_quantity"] > 0 && $item["o_status"] == 1) {
